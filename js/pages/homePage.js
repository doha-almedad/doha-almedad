...
export function renderHomePage(root){
  ...
  <section class="section home-now"><div class="container"><div class="section-head"><div><span class="eyebrow">الدوحة الآن</span><h2>ما يحدث الآن</h2></div></div><div class="home-now__group reveal-on-scroll"><div class="section-head section-head--compact"><h3>فعاليات مفتوحة للمشاركة</h3><a href="#/events" class="btn btn-ghost">كل الفعاليات</a></div><div class="grid grid-3 home-rich-grid">${events.map(ev=>`<a href="#/events/${ev.id}" class="card card--hover highlight-card home-update-card"><div class="highlight-card__meta"><span class="badge-pill badge-pill--gold">${arNum(ev.participants.length)} مشارك</span></div><h3 class="highlight-card__title">${ev.title}</h3><p>${excerpt(ev.description)}</p><div class="highlight-card__foot"><span>حتى ${new Date(ev.endDate).toLocaleDateString("ar")}</span><span>التفاصيل</span></div></a>`).join('')}</div></div>
